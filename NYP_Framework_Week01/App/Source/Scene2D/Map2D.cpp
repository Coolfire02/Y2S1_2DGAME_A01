					...
					const unsigned int uiNumCols)
{
	...
	// New way of loading -- Create the Quad Mesh using the mesh builder
	...
	// Load the player
	if (LoadTexture("Image/Scene2D_PlayerTile.tga", 200) == false)
	{
		std::cout << "Failed to load player" << std::endl;
		return false;
	}
	// Load the enemy
	if (LoadTexture("Image/Scene2D_EnemyTile.tga", 300) == false)
	{
	...
	if (LoadMap("Maps/DM2213_Map_Level_01.csv") == false)
	{
...
void CMap2D::RenderTile(const unsigned int uiRow, const unsigned int uiCol)
{
	...
	{
		if (arrMapInfo[uiCurLevel][uiRow][uiCol].value < 400)
